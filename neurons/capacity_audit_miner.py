...
class CapacityAuditMinerWorker:
    ...
    def __init__(
        ...
        *,
        ...
        # (via the worker balancer) instead of this host's own GPU. When unset,
        # the audit runs locally (default). See _use_remote_audit().
        ...
        self._audit_balancer = (
            CapacityAuditBalancerClient(self.audit_balancer_url, api_key=self.audit_balancer_api_key)
    ...
    # ── remote-audit orchestration (GPU worker via balancer) ─────────────
    # Mirrors the local waiter/runner, but the timed benchmark runs on a leased
    # worker. The HOT-START is preserved: prepare() warms the worker during the
    # B_select->B_start lead window; start() (with the B_start seed) begins the
    # timed run. All chain timing, seed derivation, signing and publishing stay
    # here on the miner; only the GPU compute is remote.
